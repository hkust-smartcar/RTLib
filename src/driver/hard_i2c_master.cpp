...
HardI2CMaster::HardI2CMaster(const I2CMaster::Config& config) :
...
void HardI2CMaster::SetBaudRate(uint32_t source_clock_Hz, uint32_t baud_rate_Hz) {
	/* Disable master mode. */
...
uint32_t HardI2CMaster::GetCyclesForWidth(uint32_t source_clock_Hz, uint32_t width_ns, uint32_t max_cycles, uint32_t prescaler) {
	assert(prescaler > 0);
...
bool HardI2CMaster::WaitForTxReady() {
...
bool HardI2CMaster::SendByteWithStartCMD(const uint8_t byte) {
...
bool HardI2CMaster::SendStopCMD() {
...
bool HardI2CMaster::SendByte(const uint8_t byte) {
	/* Wait until there is room in the fifo. */
	if (!WaitForTxReady()) {
		return false;
	}
...
bool HardI2CMaster::GetByte(uint8_t& byte) {
	uint32_t start_wait_time = Systick::GetTimeInNS();
	while (1) {
...
bool HardI2CMaster::SendByte(const uint8_t slave_addr, const uint8_t reg_addr, const uint8_t data) {
	if (!CheckForBusyBus()) {
		return false;
	}
	/* Clear all flags. */
...
bool HardI2CMaster::SendByte(const uint8_t slave_addr, const uint16_t reg_addr, const uint8_t data) {
	if (!CheckForBusyBus()) {
		return false;
	}
	/* Clear all flags. */
...
bool HardI2CMaster::SendBytes(const uint8_t slave_addr, const uint8_t reg_addr, const uint8_t* data, uint8_t size) {
	if (!CheckForBusyBus()) {
		return false;
	}
	/* Clear all flags. */
	...
	/* Turn off auto-stop option. */
	i2c_base->MCFGR1 &= ~LPI2C_MCFGR1_AUTOSTOP_MASK;
	if (!SendByteWithStartCMD((uint32_t)(slave_addr << 1u)) || !SendByte(reg_addr)) {
		return false;
	}
...
bool HardI2CMaster::SendBytes(const uint8_t slave_addr, const uint16_t reg_addr, const uint8_t* data, uint8_t size) {
	if (!CheckForBusyBus()) {
		return false;
	}
	/* Clear all flags. */
	...
	/* Turn off auto-stop option. */
	i2c_base->MCFGR1 &= ~LPI2C_MCFGR1_AUTOSTOP_MASK;
	if (!SendByteWithStartCMD((uint32_t)(slave_addr << 1u)) || !SendByte(reg_addr >> 8u) || !SendByte(reg_addr && 0xFF)) {
		return false;
	}
	for (uint8_t count = 0; count < size; ++count) {
		if (!SendByte(*(data + count))) {
...
bool HardI2CMaster::GetByte(const uint8_t slave_addr, const uint8_t reg_addr, uint8_t& data) {
	if (!CheckForBusyBus()) {
		return false;
	}
	/* Clear all flags. */
...
bool HardI2CMaster::GetByte(const uint8_t slave_addr, const uint16_t reg_addr, uint8_t& data) {
	if (!CheckForBusyBus()) {
		return false;
	}
	/* Clear all flags. */
...
bool HardI2CMaster::GetBytes(const uint8_t slave_addr, const uint8_t reg_addr, uint8_t* data, uint8_t size) {
	if (!CheckForBusyBus()) {
		return false;
	}
	/* Clear all flags. */
	...
	/* Issue command to receive data. */
	i2c_base->MTDR = 0b100000000 | LPI2C_MTDR_DATA(size - 1);
...
bool HardI2CMaster::GetBytes(const uint8_t slave_addr, const uint16_t reg_addr, uint8_t* data, uint8_t size) {
	if (!CheckForBusyBus()) {
		return false;
	}
	/* Clear all flags. */
